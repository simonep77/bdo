...
namespace Bdo.Objects
{
    ...
    public class BusinessSlot:IComparable<BusinessSlot>, IEquatable<BusinessSlot>, IDisposable
    {
        ...
        {
            return this.mProperties.ContainsKey(key) || ConfigurationManager.AppSettings[key] != null;
        }
        ...
        {
            //Carica default da appsettings
            object oRetSett = ConfigurationManager.AppSettings[key];
            object oRet = null;

            //Ritorna prima la proprieta' presa dal setting
            if (oRetSett != null && settingFirst)
                return oRetSett;
            ...
            if (oRet == null)
                throw new BusinessSlotException("PropertyGet - La proprieta' {0} non risulta impostata ne a livello di slot che di AppSettings", key);
        ...
        {
            return this.mProperties.Count + ConfigurationManager.AppSettings.Count;
        }
        ...
        public List<PropertyIdentifier> PropertyAllKeys()
        {
            ...
            if (ConfigurationManager.AppSettings != null && ConfigurationManager.AppSettings.Count > 0)
            {
                ...
                {
                    outList.Add(new PropertyIdentifier { Key = item, IsAppSetting = true });
                }
            }
        ...
        internal IDataBase DbGet(ClassSchema schema)
        {
            ...
            {
                throw new ObjectException(Resources.SessionMessages.DB_Not_Exist, schema.DbConnDef.Name);
            }
        }
        ...
        public IDataBase DbGet(string name)
        {
            ...
            if (!this.mDbList.TryGetValue(name, out db))
                throw new ObjectException(Resources.SessionMessages.DB_Not_Exist, name);
        ...
        public void DbRemove(string name, bool rollbackUnCommited)
        {
            ...
            if (name.Equals(Constants.STR_DB_DEFAULT))
                throw new ArgumentException(Resources.SessionMessages.Cannot_Delete_Default_DB);
            ...
            where T : DataObject<T>
        {
            ...
            {
                throw new ObjectException(Resources.ObjectMessages.Base_NoDbPrefixDefined, schema.ClassName, schema.TableDef.DbPrefixKey);
            }
        }
        ...
        internal DataObjectBase LoadObjectInternalByFILTER(Type origType, bool raiseNotFound, IFilter filter, OrderBy order)
        {
            ...
            if (filter == null )
                throw new BusinessSlotException(Resources.SessionMessages.LoadObj_Filter_Null);
            ...
            if (oNewObj.ObjectState != EObjectState.Loaded)
            {
                if (raiseNotFound)
                    //Richiesta eccezione
                    throw new ObjectNotFoundException(Resources.ObjectMessages.Base_Record_Filter_NotFound, oNewObj.mClassSchema.ClassName);
                else
        ...
        internal DataObjectBase LoadObjectInternalByKEY(string keyName, Type origType, bool raiseNotFound, object[] values)
        {
            ...
            if (values == null || values.Length == 0)
                throw new ObjectException(Resources.ObjectMessages.Base_NoValueForKey, origType.Name, keyName);
            ...
            if (values.Length < oKey.Properties.Count)
                throw new ObjectException(Resources.ObjectMessages.Base_KeyValuesLessThanFields, origType.Name, keyName, oKey.Properties.Count);
            ...
            if (oNewObj.mDataSchema == null)
            {
                ...
                if (oNewObj.ObjectState != EObjectState.Loaded)
                {
                    if (raiseNotFound)
                        //Richiesta eccezione
                        throw new ObjectNotFoundException(Resources.ObjectMessages.Base_RecordKeyNotFound, oNewObj.mClassSchema.ClassName, oKey.Name, ObjectHelper.ObjectEnumerableToString(values));
                    else
                        //Richiesto valore null
                        return null;
                }
        ...
        public void SaveObject<T>(T obj) where T : DataObjectBase
        {
            //Se null errore
            if (obj == null)
                throw new ObjectException(Resources.ObjectMessages.Base_Save_Null, typeof(T).Name);
        ...
        public void DeleteObject<T>(T obj) where T : DataObjectBase
        {
            //Se null errore
            if (obj == null)
                throw new ObjectException(Resources.ObjectMessages.Base_Delete_Null, typeof(T).Name);
            ...
            where TL : DataListBase
        {
            //Se null errore
            if (list == null)
                throw new ObjectException(Resources.ObjectMessages.Base_Save_Null, typeof(TL).Name);
        ...
        {
            if (other == null)
                throw new ObjectException(Resources.ObjectMessages.Base_Null_Input, typeof(T).Name);

            //Se presente tracking ritorna la medesima istanza
            if (this.LiveTrackingEnabled)
                throw new BusinessSlotException(Resources.SessionMessages.Clone_LiveTrack_Not_Allowed);
        ...
        {
            if (list == null)
                throw new ObjectException(Resources.ObjectMessages.Base_Null_Input, typeof(TL).Name);
        ...
        static void InitConfigure(SlotConfig conf)
        {
            ...
            {
                throw new BusinessSlotException(Resources.SessionMessages.Initialization_Error, ex.Message);
            }
        }
