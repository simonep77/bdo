    ...
    class ClassSchema
    {
        ...
        internal void Validate()
        {
            ...
            if (this.Properties.Count == 0)
                throw new SchemaReaderException(this, Resources.SchemaMessages.Schema_NoProperties);

            //Se la PK non contiene proprietà - errore
            if (this.PrimaryKey == null || this.PrimaryKey.Properties.Count == 0)
                throw new SchemaReaderException(this, Resources.SchemaMessages.Schema_NoPrimaryKey);
            ...
            for (int i = 0; i < this.PrimaryKey.Properties.Count; i++)
            {
                if (this.PrimaryKey.Properties[i] is PropertyObject)
                    throw new SchemaReaderException(this.PrimaryKey.Properties[i], Resources.SchemaMessages.Prop_PrimaryKey_SimpleType);
            }
            ...
            if (att is Table)
            {
                ...
                if (!string.IsNullOrEmpty(this.TableDef.DbConnectionKey))
                {
                    if (this.DbConnDef != null)
                        throw new SchemaReaderException(this, Resources.SchemaMessages.DbConnectionAlreadyDefined);
