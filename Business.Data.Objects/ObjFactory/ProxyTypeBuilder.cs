...
namespace Bdo.ObjFactory
{
    ...
    static class ProxyTypeBuilder
    {
        ...
        private static DynAssemblyProxy initProxyAssembly(Assembly ass)
        {
            ...
            ModuleBuilder pxModBuilder = assBuilder.DefineDynamicModule(STR_MODULE_NAME, true);
        ...
        internal static void BuildDaoProxyFromAssembly(ProxyAssemblyCache.ProxyAssemblyDao outProxy)
        {
            ...
            foreach (Type tOriginal in outProxy.SrcAss.GetTypes())
            {
                ...
                ADD_TYPE_ENTRY:
                    ...
                    oTypeEntry.ProxyType = typeBuild.CreateType();
                ...
                {
                    throw new SchemaReaderException("Errore dutante la scansione dell'assembly {0}. {1}", outProxy.SrcAss.FullName, ex.Message);
                }
